...
if __name__=='__main__':
    ...
    fcpo_traindata,fcpo_trainlabels,fcpo_testdata,fcpo_testlabels=build_regression_model_data(
                            fcpo_data_daily,fcpo_daily_tind,'long_spread')

    from sklearn.preprocessing import RobustScaler, MinMaxScaler
    robust_scaler=RobustScaler()
    fcpo_scaled_trainlabels=robust_scaler.fit_transform(fcpo_trainlabels)
   ...
   '''
    ...
    xgb_regressor=xgb_regressor.fit(fcpo_traindata,np.ravel(fcpo_trainlabels))
    fcpo_pred_values=xgb_regressor.predict(fcpo_testdata)
    fcpo_pred_rescaled_values=robust_scaler.inverse_transform(np.array(fcpo_pred_values).reshape(1,-1))
    ...
    mean_absolute_error(fcpo_scaled_testlabels,fcpo_pred_values)


    plt.figure(figsize=(10,10))
    fcpo_scaled_testlabels.plot()
    fcpo_pred_values.plot()
